...
The sno slider changes the amount of snow in map,
as a result of which the amount of snow in the simulator changes.

The time slider changes the simulator time.

The temperature slider changes the rain type and environmental conditions.

The rain slider changes the amount of rain.

The fog slider changes the amount of fog.
...
class World(object):
    def __init__(self, carla_world, hud, hud_wintersim, args):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    if world.is_wintersim:
                        if world.hud.help.visible:
                            world.hud.help.toggle()
                        world.hud_wintersim.help_text.toggle()
                    else:
                        if world.hud_wintersim.help_text.visible:
                            world.hud_wintersim.help_text.toggle()
                        world.hud.help.toggle()
...
class HUD(object):
    def __init__(self, width, height):
        ...
        self.help = HelpText(pygame.font.Font(mono, 16), width, height, self)
        self.server_fps = 0
        self.frame = 0
    ...
    def toggle_info(self, world):
        if world.hud_wintersim.show_info:
            if world.hud_wintersim.help.visible:
                world.hud_wintersim.help.toggle()
            world.hud_wintersim.toggle_info(world)
        if self.show_info:
...
class HUD_WINTERSIM(object):
    ...
    def toggle_info(self, world):
        world.is_wintersim = not world.is_wintersim
        if world.hud.show_info:
            if world.hud.help_text.visible:
                world.hud.help_text.toggle()
            world.hud.toggle_info(world)
        if self.show_info:
    ...
    def render(self, display, world):
        if self.show_info:
            logo = pygame.image.load('WinterSim_logo_B.png')
            logo = pygame.transform.scale(logo, (400,100))
            logo_rect = logo.get_rect()
            display_rect = display.get_rect()
            logo_rect.topright = tuple(map(lambda i, j: i - j, display_rect.topright, (10,-10))) 
...
class HelpText(object):
    ...
    def __init__(self, font, width, height, hud):
        part_of_lines = __doc__.split(";")
        if hud.__class__.__name__ == "HUD_WINTERSIM":
            lines = part_of_lines[1].split('\n')
        else:
            lines = part_of_lines[0].split('\n')
        self.font = font
        self.line_space = 18
...
def game_loop(args):
    ...
    try:
        ...
        hud_wintersim = HUD_WINTERSIM(args.width, args.height, display)
        world = World(client.get_world(), hud, hud_wintersim, args)
        ...
        while True:
            ...
            if world.is_wintersim: #if wintersim hud is activated
                for s in hud_wintersim.sliders: 
