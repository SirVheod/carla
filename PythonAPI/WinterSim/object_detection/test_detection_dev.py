...
def detect(opt, model, lidar_array, Tensor):
    try:
        dataset = KittiYOLODataset(cnf.root_dir, lidar_array, split=opt.split, mode='TEST', folder=opt.folder, data_aug=False)
...
def main():
    ...
    parser.add_argument("--model_def", type=str, default="object_detection/config/complex_tiny_yolov3.cfg", help="path to model definition file")
    parser.add_argument("--weights_path", type=str, default="object_detection/checkpoints/radar-250points-tiny-yolov3_ckpt_epoch-260_MAP-0.44.pth", help="path to weights file")
