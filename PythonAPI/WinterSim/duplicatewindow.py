...
class CameraManager(object):
    ...
    def set_sensor(self, index, notify=True):
        ...
        if needs_respawn:
            ...
            # circular reference.
            weak_self = weakref.ref(self)
            self.sensor.listen(lambda image: CameraManager.duplicate_game_window(weak_self, image))
        if notify:
    ...
    @staticmethod
    def duplicate_game_window(weak_self, image):
        self = weak_self()
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            # this runs every frame, shows current frame in cv2 window.
