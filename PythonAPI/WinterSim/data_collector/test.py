...
class SynchronyModel(object):
    ...
    def _span_player(self):
        ...
        my_vehicle = self.world.spawn_actor(my_vehicle_bp, spawn_point)
    ...
    def generate_datapoints(self, image):
        ...
        pedest = self.world.get_actors().filter('walker.*')
        actors = np.concatenate((vehicles, pedest))
        for agent in actors:
            #Distance to vehicle must be 50 or under 
            distance = lambda l: math.sqrt((l.x - player.location.x)**2 + (l.y - player.location.y)**2 + (l.z - player.location.z)**2)
            d = distance(agent.get_location())
            if agent.get_location() != self.player.get_location():
                if GEN_DATA and d <= 50 and d >= 0:
